...
async function copyAgentToProject(agent) {
  ...
  try {
    ...
    logger.error(
      `Failed to copy agent ${agent.name} to project: ${error.message}`,
    );
    return false;
  }
}
...
export async function syncCommand(options) {
  ...
  try {
    ...
    if (!options.auto) {
      ...
      agentsToRegister = unregisteredAgents.filter((a) =>
    ...
    spinner.succeed(
      `Registered ${registered} agent(s), copied ${copied} to project`,
    );
