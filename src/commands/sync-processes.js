...
export async function syncProcessesCommand(options) {
  try {
    ...
    const installedProcesses = getProcessesConfig(options.project).processes || {};
    ...
    if (existsSync(userProcessesDir)) {
      ...
      for (const entry of entries) {
        try {
          ...
          console.error(chalk.red(`Error loading process ${entry.name}: ${error.message}`));
        }
      }
    }
    ...
    availableProcesses.forEach(process => {
      const status = process.installed ? chalk.green('✓ installed') : chalk.gray('not installed');
      console.log(`  • ${process.name} ${status} - ${process.description || 'No description'}`);
    });
    
    // Step 2: Filter unregistered processes
    const unregisteredProcesses = availableProcesses.filter(p => !p.installed);
    ...
    if (!options.all && unregisteredProcesses.length > 1) {
      ...
      processesToSync = unregisteredProcesses.filter(p => selectedProcesses.includes(p.name));
    }
...
async function copyProcessToProject(process, projectProcessesDir) {
  try {
    ...
    if (process.path) {
      ...
      writeFileSync(targetPath, process.fullContent || formatProcessContent(process));
    }
    
    console.log(chalk.green(`  ✓ Copied ${process.name} to project`));
  } catch (error) {
    console.error(chalk.red(`  ✗ Failed to copy ${process.name}: ${error.message}`));
  }
}
