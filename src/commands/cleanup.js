...
export async function cleanupCommand(options) {
  ...
  try {
    ...
    if (dryRunSummary.removed.length === 0) {
      console.log(chalk.green('✓ No deprecated agents found. Your setup is up to date!'));
      return;
    }
    
    // Show deprecated agents that will be removed
    console.log(chalk.yellow(`Found ${dryRunSummary.removed.length} deprecated agent(s):\n`));
    ...
    for (const agent of dryRunSummary.removed) {
      ...
      if (replacement) {
        ...
        console.log(chalk.gray(`    → No direct replacement (functionality may be in other agents)`));
      }
    }
    
    console.log('');
    console.log(chalk.yellow('These deprecated agents are from older versions and should be removed.'));
    console.log(chalk.gray('The new agents provide improved functionality and better integration.\n'));
    ...
    if (actualSummary.errors.length > 0) {
      spinner.fail(`Cleanup completed with ${actualSummary.errors.length} error(s)`);
    ...
    console.log(`  • Removed: ${chalk.green(actualSummary.removed.length)} agent(s)`);
    console.log(`  • Config cleaned: ${actualSummary.configCleaned ? chalk.green('Yes') : chalk.gray('No')}`);
    
    if (actualSummary.removed.length > 0) {
      console.log('');
      console.log(chalk.cyan('💡 Tip: Run "claude-agents install" to see available replacement agents'));
    }
