...
async function validateAgent(agentName) {
  ...
  try {
    ...
    const agentPath = existsSync(userPath) ? userPath : 
      existsSync(projectPath) ? projectPath : null;
    ...
    const descValidation = validateDescription(frontmatter.description, agentName);
    ...
    if (frontmatter.tools) {
      const validTools = [
        ...
        'WebSearch', 'WebFetch', 'Task', 'TodoWrite', 'NotebookRead', 'NotebookEdit'
      ];
...
function displayValidationResults(results, options) {
  ...
  if (invalidAgents.length > 0 || lowScoreAgents.length > 0) {
    ...
    if (invalidAgents.length > 0) {
      console.log(chalk.red(`  • Fix critical issues in: ${invalidAgents.map(a => a.name).join(', ')}`));
    }
    
    if (lowScoreAgents.length > 0) {
      console.log(chalk.yellow(`  • Run 'claude-agents optimize' to improve: ${lowScoreAgents.map(a => a.name).join(', ')}`));
    }
  }
}
